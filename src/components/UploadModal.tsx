...
export function UploadModal({ isOpen, onClose }: UploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (thumbnailFile) {
        ...
        const thumbRes = await axios.post('/api/upload-image', thumbData);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative w-full h-full sm:h-auto sm:max-w-2xl bg-black sm:border sm:border-white/10 sm:rounded-[2.5rem] flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div 
                ...
                className="relative aspect-video bg-white/5 rounded-2xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center cursor-pointer hover:border-primary/50 transition-all group overflow-hidden shadow-inner"
              >
                <input type="file" ref={thumbInputRef} onChange={handleThumbChange} accept="image/*" className="hidden" />
                {thumbPreviewUrl ? (
                  <img src={thumbPreviewUrl} className="w-full h-full object-cover" alt="Thumbnail Preview" />
                ) : (
                  <>
                    <ImageIcon className="w-8 h-8 text-muted-foreground group-hover:text-primary transition-colors mb-2" />
                    <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Custom Thumbnail (Optional)</span>
                  </>
                )}
              </div>
            </div>
